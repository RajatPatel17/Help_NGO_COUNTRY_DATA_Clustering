import streamlit as st
import numpy as np
import pandas as pd
import joblib

# Lets Load the joblib Instances over here

with open('pipeline.joblib','rb') as file: 
    preprocess = joblib.load(file)

with open('model.joblib','rb') as file:
    model = joblib.load(file)

# Lets take input from the User 
st.title("Help NGO Organisation")
st.subheader('This Application will help NGO to classify the Country into different Development Categories using Social and Economic Factors. Original Data has been clustered using KMeans. ')

# Lets take the Inputs from the user
gdp = st.number_input('Enter the GDPPP Gdp per population')
income = st.number_input('Enter the Income per Population')
imports = st.number_input('Imports of goods and services per capita. Given as %age of the GDP per capita')
exports = st.number_input('Exports of goods and services per capita. Given as %age of the GDP per capita')
inflation = st.number_input('Inflation : The measurement of the annual growth rate of the Total GDP')

lf_expcy = st.number_input('Life Expectancy :The average number of years a new born child would live if the current mortality patterns are to remain the same')
fert = st.number_input('Fertility : The number of children that would be born to each woman if the current age-fertility rates remain the same')
health = st.number_input('Health Expend: Total health expenditure as %age of GDP')
chld_mort = st.number_input('Child Mortality: Death of children under 5 years of age per 1000 live births')

# Lets create a input list 
input_list = [chld_mort,exports,health,imports,income,inflation,lf_expcy,fert,gdp]

# Lets Create Final Input List 

final_input = preprocess.transform([input_list])

# Lets Predict the Output
if st.button('Predict the Development Category of the Country'):
    prediction = model.predict(final_input)[0]
    if prediction == 0:
        st.error('The Country falls under the Underdeveloped Country Category')
    elif prediction == 2:
        st.success('The Country falls under the Developing Country Category')
    else:
        st.success('The Country falls under the Developed Country Category')
